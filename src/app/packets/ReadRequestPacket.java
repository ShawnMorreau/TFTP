...
public class ReadRequestPacket extends Packet {
	...
	public ReadRequestPacket(String filename, int port, InetAddress address) {
		this.filename = filename;
		this.mode = "netascii"; // The default mode
		this.port = port;
		this.address = address;
	}
	
	public ReadRequestPacket(String filename, String mode, int port, InetAddress address ) {
		this.filename = filename;
		this.mode = mode;
		this.port = port;
		this.address = address;
	...
	public String toString(){
		return "01 - " + this.filename + " - 0 - " + this.mode + " - 0";
	}
	...
	public static Packet parseData(byte[] data, int port, InetAddress address) {
		...
		if (data[0] != 0 || data[1] != 1 || data.length < 6) {
		...
		if (mode.toString().toLowerCase().equals("netascii") || mode.toString().toLowerCase().equals("octet")) {
			return new ReadRequestPacket(filename.toString(), mode.toString(), port, address);
		}
